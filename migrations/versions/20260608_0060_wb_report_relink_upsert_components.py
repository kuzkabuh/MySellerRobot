...
def upgrade() -> None:
    op.add_column(
        ...
        sa.Column("rows_created_count", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "wb_daily_report_imports",
        sa.Column("rows_updated_count", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "wb_daily_report_imports",
        sa.Column("rows_unchanged_count", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "wb_daily_report_imports",
        sa.Column("rows_pending_match_count", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "wb_daily_report_imports",
        sa.Column("rows_matched_count", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "wb_daily_report_imports",
        sa.Column("rows_ambiguous_count", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "wb_daily_report_imports",
    ...
    op.add_column(
        ...
        sa.Column("stable_business_key", sa.String(length=128), nullable=True),
    )
    op.add_column(
        "wb_daily_report_rows", sa.Column("source_row_hash", sa.String(length=64), nullable=True)
    )
    op.add_column(
        "wb_daily_report_rows",
    ...
    op.add_column(
        "wb_daily_report_rows",
        sa.Column(
            "changed_fields", sa.JSON().with_variant(postgresql.JSONB, "postgresql"), nullable=True
        ),
    )
    op.add_column(
        "wb_daily_report_rows", sa.Column("srid_raw", sa.String(length=255), nullable=True)
    )
    op.add_column(
        "wb_daily_report_rows", sa.Column("srid_normalized", sa.String(length=255), nullable=True)
    )
    op.add_column(
        "wb_daily_report_rows", sa.Column("rid_normalized", sa.String(length=255), nullable=True)
    )
    op.add_column(
        "wb_daily_report_rows",
        sa.Column(
    ...
    op.add_column(
        "wb_daily_report_rows",
        sa.Column("product_match_reason", sa.String(length=255), nullable=True),
    )
    op.add_column(
        "wb_daily_report_rows",
        sa.Column("order_match_reason", sa.String(length=255), nullable=True),
    )
    op.add_column(
        "wb_daily_report_rows",
        sa.Column("order_required", sa.Boolean(), nullable=False, server_default=sa.true()),
    )
    op.add_column(
        "wb_daily_report_rows",
        sa.Column("match_attempts_count", sa.Integer(), nullable=False, server_default="0"),
    )
    op.add_column(
        "wb_daily_report_rows",
        sa.Column("last_match_attempt_at", sa.DateTime(timezone=True), nullable=True),
    )
    op.add_column("wb_daily_report_rows", sa.Column("last_match_error", sa.Text(), nullable=True))
    op.add_column(
        "wb_daily_report_rows", sa.Column("matched_at", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "wb_daily_report_rows", sa.Column("matched_order_id", sa.Integer(), nullable=True)
    )
    op.add_column(
        "wb_daily_report_rows",
    ...
    op.add_column(
        "wb_daily_report_rows", sa.Column("deleted_at", sa.DateTime(timezone=True), nullable=True)
    )

    op.execute(
        "UPDATE wb_daily_report_rows SET source_row_hash = row_hash WHERE source_row_hash IS NULL"
    )
    op.execute(
        "UPDATE wb_daily_report_rows SET srid_raw = srid, "
        "srid_normalized = lower(trim(srid)) WHERE srid IS NOT NULL"
    )
    op.execute("UPDATE wb_daily_report_rows SET order_id = linked_order_id WHERE order_id IS NULL")
    op.execute(
        "UPDATE wb_daily_report_rows SET product_id = linked_product_id WHERE product_id IS NULL"
    )
    ...
    op.create_index(
        ...
        ["stable_business_key"],
        unique=True,
        postgresql_where=sa.text("deleted_at IS NULL AND stable_business_key IS NOT NULL"),
    )
    op.create_index(
        "ix_wb_daily_report_rows_source_hash", "wb_daily_report_rows", ["source_row_hash"]
    )
    op.create_index(
        "ix_wb_daily_report_rows_stable_key", "wb_daily_report_rows", ["stable_business_key"]
    )
    op.create_index("ix_wb_daily_report_rows_srid_raw", "wb_daily_report_rows", ["srid_raw"])
    op.create_index(
        "ix_wb_daily_report_rows_srid_normalized", "wb_daily_report_rows", ["srid_normalized"]
    )
    op.create_index(
        "ix_wb_daily_report_rows_rid_normalized", "wb_daily_report_rows", ["rid_normalized"]
    )
    op.create_index("ix_wb_daily_report_rows_basket_id", "wb_daily_report_rows", ["basket_id"])
    op.create_index(
        "ix_wb_daily_report_rows_order_match_status", "wb_daily_report_rows", ["order_match_status"]
    )
    op.create_index(
        "ix_wb_daily_report_rows_order_required", "wb_daily_report_rows", ["order_required"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "report_row_id",
            sa.Integer(),
            sa.ForeignKey("wb_daily_report_rows.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
    ...
    op.create_index(
        "ix_wb_report_components_import", "wb_report_finance_components", ["report_import_id"]
    )
    op.create_index(
        "ix_wb_report_components_row", "wb_report_finance_components", ["report_row_id"]
    )
    op.create_index(
        "ix_wb_report_components_account",
        "wb_report_finance_components",
        ["marketplace_account_id"],
    )
    op.create_index("ix_wb_report_components_order", "wb_report_finance_components", ["order_id"])
    op.create_index(
        "ix_wb_report_components_product", "wb_report_finance_components", ["product_id"]
    )
    op.create_index(
        "ix_wb_report_components_category", "wb_report_finance_components", ["finance_category"]
    )
    op.create_index("ix_wb_report_components_active", "wb_report_finance_components", ["is_active"])
    op.create_index(
        "ix_wb_report_components_deleted", "wb_report_finance_components", ["deleted_at"]
    )
