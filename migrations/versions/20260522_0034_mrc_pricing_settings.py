...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "allowed_action_price_deviation_percent",
            sa.Numeric(5, 2),
            nullable=False,
            server_default=sa.text("'10.00'"),
        ),
        sa.Column(
            "auto_promo_check_enabled",
            sa.Boolean(),
            nullable=False,
            server_default=sa.text("false"),
        ),
        sa.Column(
            "auto_add_to_promotions", sa.Boolean(), nullable=False, server_default=sa.text("false")
        ),
        sa.Column(
            "auto_price_for_auto_promotions",
            sa.Boolean(),
            nullable=False,
            server_default=sa.text("false"),
        ),
        ...
        sa.UniqueConstraint(
            "user_id", "marketplace_account_id", name="uq_mrc_settings_user_account"
        ),
    )
