...
class TestListActiveAccounts:
    ...
    async def test_returns_only_active_accounts(
        ...
        active = FakeAccount(
            id=1, marketplace=Marketplace.WB, is_active=True,
            status=AccountStatus.ACTIVE, created_at=datetime.now(UTC),
        )
        session.execute = AsyncMock(
            return_value=self._make_scalars_result([active])
        )
    ...
    async def test_does_not_return_disabled_accounts(
        ...
        session.execute = AsyncMock(
            return_value=self._make_scalars_result([])
        )
    ...
    async def test_filters_by_marketplace_wb(
        ...
        session.execute = AsyncMock(
            return_value=self._make_scalars_result([wb_account])
        )
    ...
    async def test_filters_by_marketplace_ozon(
        ...
        ozon_account = FakeAccount(
            id=2, marketplace=Marketplace.OZON, is_active=True,
            status=AccountStatus.ACTIVE, created_at=datetime.now(UTC),
        )
        session.execute = AsyncMock(
            return_value=self._make_scalars_result([ozon_account])
        )
...
class TestSyncWbCommissionsHandler:
    ...
    @pytest.mark.asyncio
    async def test_no_active_accounts_shows_message(
    ...
    @pytest.mark.asyncio
    async def test_internal_error_shows_friendly_message(
        self, mock_callback: MagicMock
    ) -> None:
        from app.bot.handlers.commissions import sync_wb_commissions_handler
        ...
        with patch(
            ...
            mock_repo = MagicMock()
            mock_repo.list_active_accounts = AsyncMock(
