...
def test_fbo_order_card_is_informational() -> None:
    ...
    text = MessageFormatter().new_order_card(order, item, profit)

    assert "Ozon / FBO" in text
    assert "Действия от продавца не требуются" in text
    assert "Требует обработки продавцом" not in text
