...
- **Полностью переработан раздел «Прибыль»**: профессиональный экран финансовой аналитики для селлера WB/Ozon.
- **KPI-карточки**: 8 метрик (фактическая/плановая прибыль, отклонение, выручка, к перечислению, себестоимость, маржа, ROI) с цветовой индикацией и пояснениями.
- **Структура прибыли**: визуализация цепочки от выручки до чистой прибыли.
- **Графики**: ТОП-10 SKU по прибыли и структура расходов.
- **Блок внимания**: автоматическое выявление проблемных SKU (без себестоимости, убыточные, без фин. данных).
- **Новая таблица**: группированные колонки, цветовая индикация, компактные расходы, детализация по SKU (раскрытие строки), статус сверки с badge.
- **Пагинация**: постраничная навигация с сохранением фильтров, настройка page_size (25/50/100).
- **Быстрые фильтры + расширенные**: компактные переключатели периода, раскрываемый блок с фильтрами МП, модели, экономики, дат, сортировки.
- **Экспорт в Excel**: выгрузка текущего отчёта с фильтрами, KPI и таблицей.
- **Empty state**: профессиональный экран при отсутствии данных с кнопками синхронизации и настройки.
- **Улучшенные статусы сверки**: badge с tooltip для каждого статуса.
- **Все расчёты через Decimal**, защита от None, корректная обработка деления на ноль.

### Fixed

- Разделение плановой и фактической выручки в таблице.
- Отображение нулевых значений вместо «н/д» для пустых периодов.
...
- Центр синхронизации (Sync Center) с трекингом статусов и повторным запуском.
- Система алертов с уведомлениями в Telegram.
- WEB-кабинет: новый дизайн, план/факт, безубыточность.
- Финансовые операции Ozon.
- MasterProduct — объединённые карточки товаров WB/Ozon.
- Автоакции WB.
- Excel-импорт себестоимости.
