...
- **прибыль**: профессиональный финансовый дашборд с KPI-карточками, структурой прибыли, графиками (ТОП-10 SKU, структура расходов), таблицей с группированными колонками, пагинацией, статусами сверки, раскрытием детализации по SKU, блоком проблемных товаров и экспортом в Excel;
- план/факт, безубыточность и unit-экономика;
...
WEB_TRUSTED_ORIGINS=https://app.example.com,https://example.com,https://www.example.com
WEBHOOK_ALLOW_INSECURE_DEV=0
TRUSTED_PROXY_NETWORKS=127.0.0.1/32,172.16.0.0/12,10.0.0.0/8
...
Секреты генерируются отдельно перед заполнением `.env`:
...
случайной строкой. `ENCRYPTION_KEY` обязан быть Fernet-ключом: production installer
проверяет его через `cryptography.fernet.Fernet` и остановит установку при ошибке.
Если база уже содержит подключённые WB/Ozon кабинеты, менять `ENCRYPTION_KEY` нельзя:
старые marketplace API-ключи перестанут расшифровываться.

### Production domain configuration for example.com
...
BOT_WEBHOOK_SECRET=
BOT_WEBHOOK_ENABLED=true
YOOKASSA_RETURN_URL=https://app.example.com/payment/success
...
Домен `bot.example.com` должен проксировать только технические endpoints бота,
...
Публичный лендинг `https://example.com/` лежит в `public/index.html`.
Стили и скрипты находятся в `public/assets/css/main.css` и
`public/assets/js/main.js`. Технические файлы для индексации расположены рядом:
`public/robots.txt`, `public/sitemap.xml`, `public/manifest.webmanifest`,
`public/favicon.svg`.

На production публичный домен отдаётся host-side nginx из шаблона
`deploy/nginx/mpcontrol.conf.template`: `example.com` и
`www.example.com` читают файлы из `${PROJECT_DIR}/public`, а
`app.example.com` проксируется в FastAPI web-кабинет.
...
curl -L https://example.com/ | head -80
curl -I https://www.example.com/
curl -I https://app.example.com/web/
```
...
`https://bot.example.com/webhook/telegram`. Если задан `BOT_WEBHOOK_SECRET`,
...
```text
Telegram -> bot.example.com:443 -> nginx -> 127.0.0.1:8000 -> FastAPI /webhook/telegram
```
...
sudo grep -R "bot.example.com" -n \
  /etc/nginx/sites-enabled /etc/nginx/conf.d /opt/mpcontrol/deploy/nginx 2>/dev/null

curl -4 -vkI https://bot.example.com/health
curl -4 -vkI https://bot.example.com/webhook/telegram
...
https://example.com/web/settings?tab=subscription
https://example.com/web/settings?tab=profile
```
...
`/web/settings?tab=sync`, а администратору также доступен контроль запусков на
...
- `/web/admin/users/{id}` — карточка пользователя, кабинеты WB/Ozon, последние
  заказы, платежи, ошибки, уведомления и аудит действий.
...
Production-ротация file logs закреплена в `deploy/logrotate/mpcontrol`: активное окно
логов — 48 часов, старые файлы сжимаются с датированным суффиксом.
...
- `deploy/nginx/bot.example.com.conf` — отдельный nginx config для Telegram webhook;
- `deploy/systemd/` — templates для безопасного Telegram-trigger деплоя.

Перед крупными миграциями делайте backup БД.

### Установка на новый Ubuntu 24.04 сервер
...
```bash
cd /root
curl -fsSL https://raw.githubusercontent.com/kuzkabuh/MySellerRobot/main/deploy/install.sh -o install.sh
chmod +x install.sh
SSL_EMAIL=admin@example.com ./install.sh
```

Если `.env` отсутствует, installer создаст его из `.env.example`, выставит права
`600` и остановится на валидации placeholder-значений. Заполните реальные секреты,
проверьте `BOT_WEBHOOK_ENABLED=true`, затем запустите installer повторно. Существующий
`.env` никогда не перезаписывается.

Installer выполняет:

- установку Docker, Nginx, Certbot и `python3-cryptography`;
- проверку обязательных env-переменных, `APP_SECRET_KEY` и Fernet `ENCRYPTION_KEY`;
- подготовку `alembic_version.version_num` как `VARCHAR(255)` до миграций;
- сборку production image и запуск PostgreSQL, Redis, API, bot и worker;
- выпуск SSL для public/app/bot доменов;
- проверку, что сертификат для `bot.example.com` содержит SAN `DNS:bot.example.com`;
- настройку Telegram webhook;
- включение `mpcontrol-backup.timer`.

Certbot должен выпускать сертификат, в SAN которого есть все production-домены:
...
Если не-критичный домен ещё не указывает на текущий сервер, installer пропустит его
для текущего Certbot-запуска и выведет предупреждение. `bot.example.com`
критичен: без A-записи на текущий сервер installer остановится, потому что Telegram
не примет webhook с сертификатом без `DNS:bot.example.com`.

### Бэкапы и восстановление

Автоматический backup запускается systemd timer один раз в сутки:

```bash
systemctl status mpcontrol-backup.timer --no-pager
journalctl -u mpcontrol-backup.service -n 100 --no-pager
```

Ручной запуск:

```bash
/opt/mpcontrol/scripts/backup.sh
```

Архивы складываются в `/opt/mpcontrol/backups/daily`. В backup входят PostgreSQL dump
и архив важных файлов проекта, включая `.env`, `docker-compose.prod.yml`, `deploy/`,
`runtime/`, `uploads/` и `storage/` при наличии. Доступ к каталогу backup должен быть
ограничен root/mpcontrol, потому что `.env` содержит секреты.
В production файловый архив с `.env` должен быть зашифрован через
`BACKUP_ENCRYPTION_ENABLED=1`, если только администратор явно не включил
`BACKUP_ALLOW_PLAINTEXT_SECRETS=1`.

Восстановление БД из полного архива:

```bash
cd /opt/mpcontrol
RESTORE_CONFIRM=YES ./scripts/restore.sh /opt/mpcontrol/backups/daily/mpcontrol_full_YYYY-MM-DD_HH-MM-SS.tar.gz
docker compose -f docker-compose.prod.yml up -d
```

Чтобы дополнительно восстановить `.env` и runtime/config файлы из архива:

```bash
RESTORE_CONFIRM=YES RESTORE_FILES=1 ./scripts/restore.sh /opt/mpcontrol/backups/daily/mpcontrol_full_YYYY-MM-DD_HH-MM-SS.tar.gz
```

Перед restore скрипт создаёт safety dump в `/opt/mpcontrol/backups/restore`.
...
9. Проверить Telegram webhook на `https://bot.example.com/webhook/telegram`.
...
docker compose -f docker-compose.prod.yml ps
docker compose -f docker-compose.prod.yml exec api alembic current
echo | openssl s_client -connect bot.example.com:443 -servername bot.example.com 2>/dev/null | openssl x509 -noout -issuer -subject -dates -ext subjectAltName
curl -s "https://api.telegram.org/bot$BOT_TOKEN/getWebhookInfo"
curl -I https://bot.example.com/health
curl -I https://bot.example.com/webhook/telegram
docker compose -f docker-compose.prod.yml logs -f api bot worker --tail=200
```
