...
Релиз `1.6.4` завершает несколько незакрытых частей интеграций:

- Ozon SellerInfo сохраняется в карточке подключённого кабинета и отображается в Telegram/WEB;
- добавлены справочник складов Ozon, складские FBS-остатки, снимки цен и участие товаров в акциях;
- Ozon stock sync поддерживает cursor-пагинацию и не ограничивается первыми 1000 строками;
- polling Ozon-заказов использует `last_order_poll_at` с overlap, чтобы не терять заказы после простоя;
- ежедневно в 05:00 МСК worker загружает WB `/api/v1/supplier/sales` за D-1/D-2/D-3 с `flag=1`;
- повторная загрузка WB sales обновляет предварительные значения `forPay`, `finishedPrice` и возвраты.

## Release 1.6.3 — жизненный цикл подписок

Релиз `1.6.3` развивает подписочную модель без перехода к старым `SubscriptionPlan` /
`Subscription`:

- `monthly` и `yearly` явно передаются в `PaymentService` и `SubscriptionService`;
- месячная подписка длится 30 дней, годовая — 365 дней;
- продление того же тарифа до окончания периода добавляет дни к текущему `expires_at`;
- продление истёкшей подписки считается от текущей даты;
- `TRIAL` считается активным статусом, но истёкший trial возвращает пользователя на FREE;
- один пользователь может получить trial только один раз;
- upgrade BASIC → PRO заменяет старую активную подписку статусом `REPLACED` и создаёт новую PRO;
- истёкшие `ACTIVE` / `TRIAL` подписки можно закрыть через
  `SubscriptionService.expire_outdated_subscriptions()`;
- добавлена миграция `20260517_0013_subscription_lifecycle` с полем `period` у подписки и
  статусом `REPLACED`.

## Telegram UX polish

Сообщения Telegram-бота приводятся к единому пользовательскому стилю:

- списки заказов, остатков, FBS-рисков и ошибок синхронизации оформляются карточными блоками;
- длинные внешние ID сокращаются в списках и показываются в `<code>`;
- маркетплейсы отображаются как `Wildberries` и `Ozon`;
- даты, суммы, проценты и статусы форматируются единообразно;
...
- Telegram-бот переведён на централизованный `parse_mode=HTML`, а динамические значения в
  подписках, карточках заказов, кабинетах, себестоимости и админских diagnostics экранируются
  перед вставкой в HTML-сообщения;
- тексты подписок и тарифов собираются через единый formatter: каталог тарифов, карточки
  FREE/BASIC/PRO/ENTERPRISE, текущая подписка, помощь и уведомление о смене тарифа;
- администратор из `ADMIN_TELEGRAM_IDS` может вручную назначать FREE, BASIC, PRO и ENTERPRISE
  себе или пользователю по Telegram ID через `🛠 Администрирование` →
  `💳 Управление тарифами` либо команду `/admin_tariffs`;
- ручное назначение тарифа использует `SubscriptionTier` и `UserSubscription`, не создаёт
  фиктивные `Payment` и логирует события `admin_tariff_*`;
- web-кабинет получил Material-style design tokens, единые KPI/cards/tables/badges и smoke-тесты
  базовой оболочки;
...
WEB-кабинет стал самостоятельной рабочей панелью селлера. Основные разделы:

- `Главная` — приветствие, тариф, подключённые кабинеты, KPI, динамика выручки и прибыли;
- `Заказы`, `Продажи`, `Возвраты` — операционные события WB/Ozon с фильтрами и таблицами;
- `Прибыль`, `План/факт`, `Безубыточность`, `Себестоимость` — управленческая экономика,
  отклонения, расчёт безопасной цены и историческое редактирование себестоимости;
- `Товары`, `Сопоставление WB / Ozon`, `Остатки` — каталог MasterProduct, ручное сопоставление,
  прогноз out-of-stock;
- `Алерты`, `Качество данных`, `Контроль ошибок`, `Аналитика` — центр внимания, качество данных
  и обзор бизнеса за 30 дней;
- `Кабинеты маркетплейсов`, `Подписка и тариф`, `Профиль и настройки` — аккаунт пользователя,
  лимиты тарифа, платежи, статусы синхронизации и профильные настройки.

Авторизация WEB по-прежнему выполняется через одноразовую Telegram-ссылку. Базовые операции
аналитики и редактирование себестоимости доступны прямо в WEB, а подключение новых кабинетов и
оформление подписки остаются в Telegram-боте.
...
- в администрирование добавлен раздел `💳 Управление тарифами`: админ может вручную назначить
  себе или пользователю по Telegram ID тариф FREE, BASIC/PRO на 30 или 365 дней, либо бессрочный
  ENTERPRISE без создания фиктивного платежа;
...
6. `🛠 Администрирование` → `💳 Управление тарифами` — позволяет вручную назначить тариф себе
   или найденному пользователю.
