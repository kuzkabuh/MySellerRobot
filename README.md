...
Текущая версия после Этапа 7: `1.4.17`. Версия хранится в `VERSION` и в
`pyproject.toml`.
...
- web-login по `/web/login?token=...` создаёт полноценную session cookie с `Path=/web`, поэтому
...
- добавлен compatibility route `/web/web/login`, чтобы старый неверный nginx proxy path не
  отдавал `{"detail":"Not Found"}` до обновления конфигурации;
