...
Публичный лендинг `https://mpcontrol.online/` лежит в `public/index.html`.
...
`deploy/nginx/mpcontrol.conf.template`: `mpcontrol.online` и
`www.mpcontrol.online` читают файлы из `${PROJECT_DIR}/public`, а
`app.mpcontrol.online` проксируется в FastAPI web-кабинет.
...
```bash
curl -I https://mpcontrol.online/
curl -L https://mpcontrol.online/ | head -80
curl -I https://www.mpcontrol.online/
curl -I https://app.mpcontrol.online/web/
```
...
```text
mpcontrol.online      -> SERVER_IP
www.mpcontrol.online  -> SERVER_IP
app.mpcontrol.online  -> SERVER_IP
api.mpcontrol.online  -> SERVER_IP
bot.mpcontrol.online  -> SERVER_IP
```
...
- проверку, что сертификат для `bot.mpcontrol.online` содержит SAN `DNS:bot.mpcontrol.online`;
...
```text
DNS:mpcontrol.online
DNS:www.mpcontrol.online
DNS:app.mpcontrol.online
DNS:api.mpcontrol.online
DNS:bot.mpcontrol.online
```

Если не-критичный домен ещё не указывает на текущий сервер, installer пропустит его
для текущего Certbot-запуска и выведет предупреждение. `bot.mpcontrol.online`
критичен: без A-записи на текущий сервер installer остановится, потому что Telegram
не примет webhook с сертификатом без `DNS:bot.mpcontrol.online`.
...
docker compose -f docker-compose.prod.yml ps
docker compose -f docker-compose.prod.yml exec api alembic current
echo | openssl s_client -connect bot.mpcontrol.online:443 -servername bot.mpcontrol.online 2>/dev/null | openssl x509 -noout -issuer -subject -dates -ext subjectAltName
