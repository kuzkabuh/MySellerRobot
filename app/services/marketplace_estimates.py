"""version: 1.0.0
description: Baseline marketplace expense estimates for planned order economics.
updated: 2026-05-15
"""
...
DEFAULT_WB_FBS_COMMISSION_RATE = Decimal("0.33")
...
def estimate_marketplace_expenses(order: Order, item: OrderItem) -> ExpenseEstimate:
    ...
    if (
        ...
        commission_rate = DEFAULT_WB_FBS_COMMISSION_RATE
...
def calculate_planned_economics(order: Order, item: OrderItem) -> PlannedEconomics:
    """Calculate display-safe planned profit with baseline estimates when needed."""

    expenses = estimate_marketplace_expenses(order, item)
    revenue = quantize_money((item.discounted_price or ZERO) * Decimal(item.quantity or 1))
